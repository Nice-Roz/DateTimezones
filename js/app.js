...
let favoriteTimeZones = [
    // {country: "Andora", timezone:"Europe/Andorra" },
    // {country: "UAE(dubai)", timezone:"Asia/Dubai" },
    // {country: "Andora", timezone:"Europe/Andorra" }
];
...
function getCurrentDateTime(timezone,elementId){
//     const date = new Date();
//     const result = date.getHours()+": " +date.getMinutes() + ": " + date.getSeconds(); 
...
// Function For loading Timezones
function loadTimeZoneList(list){
timeZoneList.innerHTML='';
    // for showing arrays its better to use Foreach loop , for controlling more
...
`
{/* <td><button onclick="addFavorite('${item.country}')">Add</button></td> */}
...
function loadFavoriteTimezoneList(){
    ...
    favoriteTimeZones.forEach((item,index)=>{
       const row = document.createElement('li')
       row.innerHTML=`
       ${item.country} <span id='time-${index}'>
        ${getCurrentDateTime(item.timezone)}
       </span>
       <button class="remove-btn" onclick="removeFavorite(${index})">Remove</button>
       `
       favoriteTimeZonesList.appendChild(row);
    //     ${getCurrentDateTime(item.timezone,`time-${index}`)} 
    })
}
}
